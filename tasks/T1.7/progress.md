...
## Verification
- [ ] Rust unit tests: given (battery, link set, payload size) -> deterministic transport pick matching a documented decision table in `ARCHITECTURE.md`
- [ ] Grep proves Swift/Kotlin routers no longer contain independent preference ordering (only availability checks)
